...
import {ingoreColumns} from "./columns";
...
export default function IgnoreTable({ id }: { id: number }) {
  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);
  const [perPage, setPerPage] = useState(10);
  const { data, refetch } = useQuery({
    queryKey: ["activity", id],
    queryFn: () => messageService.getActivity(id, perPage, page, search),
  });
  const table = useReactTable({
    data: data?.data || [],
    columns: ingoreColumns,
